...
近期，发现一个不错的提示词，原来还可以这样玩。只需提供一个概念词即可生成可视化非常棒的思维笔记。对于我这种手残党来说，感觉太强了。思维笔记具备卓越的可视化功能，能够以直观、生动的方式呈现核心要点，极大地提升了信息传达与理解的效率。如今可以通过 AI 实现批量生产，在知识传播与内容运营方面应该具备相当可观的前景，比如做个相关的小红书号？

以下是生成的图片，效果杠杠的，ChatGPT4o 的强大能力。感兴趣的朋友可以试试，免费的 ChatGPT4o 就可以实现，提示词已经放到后面了。

![[UlacbG97hoPtB6xlBu8ce9tinQh.png]]
![[M7FzbBA4dodvXCxwW1AcjyBEnRe.png]]
![[EYcIbxtCtoo5OExATrYcZBuJnNe.png]]
...
```
就“{{topic}}”主题创建简洁、直观的笔记。注释必须清晰适合{{orientation}}版式（横向/纵向），具有以下特点：
字体大小适中： 舒适的可读性。
...
使用记号笔风格，模拟注释、更正和其他类似自发批注的古怪涂鸦。

加入与主题相关的拼贴式照片摘录，并在其上进行批注或涂鸦。

- 语言文本准确性限制（严格）：

   - 生成“{{语言}}”文本时，请遵守公认的词典和标准语法规则。

   - 对于中文（汉语）或其他复杂脚本语言：

     - 确保每个字符或符号都正确、标准、使用得当。

     - 仔细检查笔画顺序，避免不存在的变体，并在最终确定注释前核实用法。
...
- 插图风格： 详细手绘、简约草图或带注释的杂志/照片剪贴。

设置好参数后，就可以用所选语言生成注释，并严格遵守所选格式和视觉指南。
```
